...
def create_comparison_for_sports_images():
    ...
    output_dir = os.path.join(os.path.dirname(
        __file__), "sports_comparison_output")
    ...
    try:
        ...
        draw.text((original_img.width + 20, 10),
                  "Sports Action (JPEG)", fill=(0, 0, 0), font=font)
        draw.text((original_img.width * 2 + 30, 10),
                  "Sports Action RAW", fill=(0, 0, 0), font=font)

        # Add processing details
        draw.text((10, 30), "No processing", fill=(100, 100, 100), font=font)
        draw.text((original_img.width + 20, 30),
                  f"Exp: +0.04, Shadows: +10, Midtone Protection", fill=(100, 100, 100), font=font)
        draw.text((original_img.width * 2 + 30, 30),
                  f"Exp: +0.02, Shadows: +5, Midtone Protection", fill=(100, 100, 100), font=font)

        # Save comparison
        comparison_path = os.path.join(
            output_dir, f"{filename}_sports_comparison.jpg")
        comparison.save(comparison_path, 'JPEG', quality=90)
        print(f"✅ Comparison saved: {comparison_path}")

        # Analyze brightness in different regions
        analyze_brightness_regions(
            original_img, sports_result, sports_raw_result)
...
def analyze_brightness_regions(original, sports_action, sports_raw):
    ...
    print(f"   🏖️  Bottom Region (sand/ground area):")
    print(f"      Original:     {orig_bottom:.1f}")
    print(
        f"      Sports Action: {sports_bottom:.1f} ({sports_bottom - orig_bottom:+.1f})")
    print(
        f"      Sports RAW:   {raw_bottom:.1f} ({raw_bottom - orig_bottom:+.1f})")
    ...
    print(f"   🌍 Overall Image:")
    print(f"      Original:     {orig_overall:.1f}")
    print(
        f"      Sports Action: {sports_overall:.1f} ({sports_overall - orig_overall:+.1f})")
    print(
        f"      Sports RAW:   {raw_overall:.1f} ({raw_overall - orig_overall:+.1f})")

    # Calculate improvement ratio
    sports_ratio = (sports_bottom - orig_bottom) / (sports_overall -
                                                    orig_overall) if sports_overall != orig_overall else 0
    raw_ratio = (raw_bottom - orig_bottom) / (raw_overall -
                                              orig_overall) if raw_overall != orig_overall else 0

    print(f"\n🎯 Sand Protection Effectiveness:")
    print(f"   Lower values = better sand protection (less brightening in ground areas)")
