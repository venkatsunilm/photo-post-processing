...
from utils.raw_processing import load_image_smart
import sys
import os
import time
...
def test_preset_impact(img, preset_name):
    ...
    try:
        ...
        # Calculate changes
        brightness_change = enhanced_props['brightness'] - \
            original_props['brightness']
        contrast_change = enhanced_props['contrast'] - \
            original_props['contrast']
        ...
        r_change = enhanced_props['color_balance']['r'] - \
            original_props['color_balance']['r']
        g_change = enhanced_props['color_balance']['g'] - \
            original_props['color_balance']['g']
        b_change = enhanced_props['color_balance']['b'] - \
            original_props['color_balance']['b']

        print(f"   🎨 Color shifts:")
...
def compare_presets_on_image(image_path):
    ...
    print(f"🔬 PRESET COMPARISON TEST")
    ...
    successful_presets = {k: v for k,
                          v in results.items() if v.get('success', False)}

    if successful_presets:
        # Find extremes
        fastest = min(successful_presets.items(),
                      key=lambda x: x[1]['processing_time'])
        slowest = max(successful_presets.items(),
                      key=lambda x: x[1]['processing_time'])

        brightest = max(successful_presets.items(),
                        key=lambda x: x[1]['brightness_change'])
        most_contrast = max(successful_presets.items(),
                            key=lambda x: x[1]['contrast_change'])
...
def performance_benchmark():
    ...
    for file_path in test_files[:5]:  # Test first 5 files
        ...
        try:
            ...
            print(
                f"   ✅ Loaded in {load_time:.2f}s ({img.size[0]}x{img.size[1]})")

            # Test one preset for processing speed
            preset_start = time.time()
            enhanced = apply_photoshop_preset(img, 'sports_action')
    ...
    if successful_loads > 0:
        ...
        print(f"\n📊 BENCHMARK RESULTS:")
        print(
            f"   ✅ Success rate: {successful_loads}/{len(test_files[:5])} files")
