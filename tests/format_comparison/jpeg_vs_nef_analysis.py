...
def compare_format_processing(jpeg_file, nef_file, preset_name="sports_action"):
    ...
    try:
        ...
        start_time = time.time()
        jpeg_processed, jpeg_history = apply_photoshop_preset(jpeg_orig, preset_name)
        jpeg_process_time = time.time() - start_time

        start_time = time.time()
        nef_processed, nef_history = apply_photoshop_preset(nef_orig, preset_name)
        ...
        nef_processed_props = analyze_image_properties(nef_processed, "NEF Processed")
        ...
        return {
            ...
            "processing_history": {"jpeg": jpeg_history, "nef": nef_history},
        }
...
def print_image_comparison(props1, props2):
    ...
    print(
        f"{'Mean Brightness':<25} {jpeg_brightness:<20.1f} {nef_brightness:<20.1f} {brightness_diff:+.1f}"
    )

    # Dynamic range utilization
    dr_diff = props2["dynamic_range_utilization"] - props1["dynamic_range_utilization"]
    print(
...
def analyze_processing_impact(original_props, processed_props, format_name):
    ...
    brightness_change = ((proc_brightness - orig_brightness) / orig_brightness) * 100

    print(f"   Brightness change: {brightness_change:+.1f}%")

    # Dynamic range change
    dr_change = (
        processed_props["dynamic_range_utilization"]
        - original_props["dynamic_range_utilization"]
    )
    dr_change_percent = (dr_change / original_props["dynamic_range_utilization"]) * 100
...
def provide_format_recommendations(jpeg_props, nef_props, preset_name):
    ...
    if nef_dr > jpeg_dr * 1.2:
        print(
            "   🎯 NEF has significantly more dynamic range - use more aggressive presets"
        )
        print(
            "   📈 Consider 'sports_action_raw' instead of 'sports_action' for NEF files"
        )
