...
class PhotoshopStyleEnhancer:
    ...
    def exposure_adjustment(self, exposure_value: float) -> "PhotoshopStyleEnhancer":
        """
    ...
    def highlights_shadows(
        self,
        highlights: Union[int, float] = -20,
        shadows: Union[int, float] = 20,
        range_mask: Union[int, float] = 50,
    ) -> "PhotoshopStyleEnhancer":
        """
        ...
        for c in range(3):
            # Adjust highlights
            img_array[:, :, c] = (
                img_array[:, :, c] * (1 - highlight_mask)
                + (img_array[:, :, c] * highlight_factor) * highlight_mask
            )

            # Adjust shadows
            img_array[:, :, c] = (
                img_array[:, :, c] * (1 - shadow_mask)
    ...
    def vibrance_saturation(
        self, vibrance: Union[int, float] = 0, saturation: Union[int, float] = 0
    ) -> "PhotoshopStyleEnhancer":
        """
        ...
        if vibrance != 0:
            ...
            vibrance_factor = 1.0 + (vibrance / 100.0) * vibrance_mask[:, :, np.newaxis]
    ...
    def clarity_structure(
        self, clarity: Union[int, float] = 0, structure: Union[int, float] = 0
    ) -> "PhotoshopStyleEnhancer":
        """
        ...
        if clarity != 0:
            ...
            blurred = self.working.filter(ImageFilter.GaussianBlur(radius=radius))
        ...
        if structure != 0:
            ...
            if structure > 0:
                unsharp_filter = ImageFilter.UnsharpMask(
                    radius=radius, percent=int(amount * 200), threshold=threshold
                )
    ...
    def color_temperature(
        self, temperature: Union[int, float] = 0, tint: Union[int, float] = 0
    ) -> "PhotoshopStyleEnhancer":
        """
    ...
    def portrait_enhancements(
        self,
        skin_smoothing: Union[int, float] = 0,
        eye_brightness: Union[int, float] = 0,
        teeth_whitening: Union[int, float] = 0,
    ) -> "PhotoshopStyleEnhancer":
        """
        ...
        if skin_smoothing > 0:
            ...
            blurred = self.working.filter(ImageFilter.GaussianBlur(radius=blur_radius))
            ...
            if smooth_factor > 0.3:  # Only for higher smoothing values
                detail_image = ImageEnhance.Sharpness(self.working).enhance(1.1)
                self.working = Image.blend(self.working, detail_image, 0.2)

        self.history.append(f"Portrait: Smoothing: {skin_smoothing}")
        return self

    def apply_midtone_protection(self) -> "PhotoshopStyleEnhancer":
        """
        ...
        bright_midtone_percentage = np.sum(bright_midtone_mask) / brightness.size
        ...
        should_protect = (
            bright_midtone_percentage > 0.15  # At least 15% bright midtones
            and overall_brightness > 120  # Overall bright image
        )
        ...
        if should_protect:
            ...
            if np.any(target_mask):
                ...
                protected_brightness = np.clip(protected_brightness, 0, 255).astype(
                    np.uint8
                )
                ...
                self.history.append("Midtone Protection: No target areas found")
        else:
            reason = (
                "low bright areas"
                if bright_midtone_percentage <= 0.15
                else "image not bright enough"
            )
    ...
    def reset(self) -> "PhotoshopStyleEnhancer":
...
def apply_photoshop_preset(
    image: Image.Image, preset_name: str
    ...
    enhancer.highlights_shadows(
        highlights=preset.get("highlights", 0), shadows=preset.get("shadows", 0)
    )
    enhancer.vibrance_saturation(
        vibrance=preset.get("vibrance", 0), saturation=preset.get("saturation", 0)
    )
    enhancer.clarity_structure(
        clarity=preset.get("clarity", 0), structure=preset.get("structure", 0)
    )
    enhancer.color_temperature(
        temperature=preset.get("temperature", 0), tint=preset.get("tint", 0)
    )
    enhancer.portrait_enhancements(skin_smoothing=preset.get("skin_smoothing", 0))
